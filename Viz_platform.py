import pandas as pd
import plotly.graph_objs as go
import dash
import dash_core_components as dcc
import dash_html_components as html
import plotly.figure_factory as ff
import plotly.express as px
import pandas_profiling


markdown_text = '''
    ![Image](data:image/png;base64,)
    '''

markdown_text_1 = '''
    ![Image](https://media0.giphy.com/media/3oKIPEqDGUULpEU0aQ/giphy.gif)
    '''

df = pd.read_csv('dataset/sample_data1.csv', encoding='ISO-8859-1')
df1 = df.COUNTRY.value_counts().reset_index()
df1.columns = ['country', 'counts']
# print(df1)

df2 = df.State.value_counts().reset_index()
df2.columns = ['state', 'counts']
# print(df2)

df3 = df.groupby('Industry')[['FTE']].mean().reset_index()
# print(df3)

df4 = df.groupby('State')[['FTE']].mean().reset_index()
# print(df4)

df5 = df.groupby(['State']).mean().reset_index()
#print(df5)

df6 = df.groupby(['Industry']).mean().reset_index()
#print(df6)

df7 = pandas_profiling.ProfileReport(df)
#df7.to_file('Report.html')

app = dash.Dash()

app.layout = html.Div(children =[
    dcc.Markdown(children=markdown_text, style={'textAlign': 'center'}),
    html.H1('Visualization Platform', style={'textAlign': 'center'}),
    dcc.Markdown(children=markdown_text_1, style={'textAlign': 'center'}),
    dcc.Graph(id='pie',
              figure={
                  'data': [go.Pie(labels=df1['country'],
                                  values=df1['counts'])],
                  'layout': go.Layout(title='Country wise data')
              }
              ),
    dcc.Graph(id='Box',
              figure={
                  'data': [go.Box(x=df['COUNTRY'],
                                  y=df['FTE'])],
                  'layout': go.Layout(title='Average FTE per country',
                                      xaxis={'title': 'Country'},
                                      yaxis={'title': 'FTE'}
                                      )
              }
              ),
    dcc.Graph(id='Bar',
              figure={
                  'data': [go.Bar(x=df2['state'],
                                  y=df2['counts'],
                                  marker={'color': 'tan'})],

                  'layout': go.Layout(title='State wise company or CEO Count',
                                      xaxis={'title': 'State'},
                                      yaxis={'title': 'Company count'})
              }
              ),
    dcc.Graph(id='Bar_1',
              figure={
                  'data': [go.Bar(x=df['Industry'],
                                  y=df['FTE'],
                                  marker={'color': 'mediumslateblue'})],

                  'layout': go.Layout(title='Total FTE per industry',
                                      xaxis={'title': 'Industry'},
                                      yaxis={'title': 'Sum of FTE'})
              }
              ),
    dcc.Graph(id='Violin',
              figure={
                  'data': [go.Violin(
                      x=df['Industry'],
                      y=df['FTE'],
                      box_visible=True,
                      # line_color= 'black',
                      meanline_visible=True,
                      # fillcolor='orange',
                      # opacity=0.4,
                      # color= df['COUNTRY'],
                      # box=True,
                      # marker = {'color': 'mediumslateblue'}
                  )],

                  'layout': go.Layout(title='Average FTE per industry',
                                      xaxis={'title': 'Industry'},
                                      yaxis={'title': 'Average FTE'})
              }
              ),
    dcc.Graph(id='Horizontal bar',
              figure={
                  'data': [go.Bar(
                      y=df4['State'],
                      x=df4['FTE'],
                      orientation='h',
                      marker={'color': 'orange'}  # lightcoral
                  )],

                  'layout': go.Layout(title='Average FTE per State',
                                      xaxis={'title': 'Average FTE'},
                                      yaxis={'title': 'Industry'})
              }
              ),
    dcc.Graph(id='Scatter plot',
              figure={
                  'data': [go.Scatter(
                      x=df['Company_age'],
                      y=df['FTE'],
                      text = df['Company'],
                      mode='markers'
                  )],

                  'layout': go.Layout(title='Age wise FTE growth',
                                      xaxis={'title': 'Company age'},
                                      yaxis={'title': 'FTE'})
              }
              ),
    dcc.Graph(id='line chart',
              figure={
                  'data': [
                      {'x': df5['State'], 'y': df5['6Months_growth'], 'type': 'line', 'name': '6month growth'},
                      {'x': df5['State'], 'y': df5['1Year_growth'], 'type': 'line', 'name': '1year growth '},
                      {'x': df5['State'], 'y': df5['2Year_growth'], 'type': 'line', 'name': '2year growth '}],

                  'layout': {'title': 'State wise FTE growth',
                             'xaxis':{'title': 'State'},
                             'yaxis': {'title': 'Avg FTE growth in percentile'}
                             }
              }),

    dcc.Graph(id='line chart_2',
              figure={
                  'data': [
                      {'x': df6['Industry'], 'y': df6['6Months_growth'], 'type': 'line', 'name': '6month growth'},
                      {'x': df6['Industry'], 'y': df6['1Year_growth'], 'type': 'line', 'name': '1year growth '},
                      {'x': df6['Industry'], 'y': df6['2Year_growth'], 'type': 'line', 'name': '2year growth '}],

                  'layout': {'title': 'Industry wise FTE growth',
                             'xaxis':{'title': 'Industry'},
                             'yaxis': {'title': 'Avg FTE growth in percentile'}
                             }
              })
]
)


if __name__ == '__main__':
    app.run_server()
